...
from cs336_basics import tokeniser, transformer, training_utils
...
def run_rope(
    ...
    """
    rope = transformer.RotaryPositionalEmbedding(theta, d_k, max_seq_len, in_query_or_key.device)
...
def run_get_batch(
    ...
    """
    return training_utils.get_batch(dataset, batch_size, context_length, device)
...
def run_cross_entropy(
    ...
    """
    return training_utils.cross_entropy_loss(inputs, targets)
...
def run_gradient_clipping(parameters: Iterable[torch.nn.Parameter], max_l2_norm: float) -> None:
    ...
    """
    return training_utils.grad_clip(parameters, max_l2_norm)
...
def run_get_lr_cosine_schedule(
    ...
    """
    return training_utils.learning_rate_schedule(it, max_learning_rate, min_learning_rate, warmup_iters, cosine_cycle_iters)
...
def run_save_checkpoint(
    ...
    """
    training_utils.save_checkpoint(model, optimizer, iteration, out)
...
def run_load_checkpoint(
    ...
    """
    return training_utils.load_checkpoint(model, optimizer, src)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    return tokeniser.train_bpe(input_path, vocab_size, special_tokens)
